...
/**
 * Database-backed admin login — sets HttpOnly cookie so /api/admin/* routes can authenticate.
 * Client-only localStorage auth cannot satisfy server-side requireAdminAuth.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isAdminSessionSigningAvailable()) {
      console.error('[admin/login] Set ADMIN_SESSION_SECRET or NEXTAUTH_SECRET (16+ chars)')
