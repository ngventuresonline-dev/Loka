  ...
  if (catCount === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-8 text-center">
        ...
        <div>
          <div className="text-sm text-gray-400">No {filter} outlets mapped here yet</div>
          <div className="text-[11px] text-gray-600 mt-1">Try another zone on the map</div>
        </div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        <div className="rounded-lg bg-white/[0.04] border border-white/[0.05] px-2 py-2.5 text-center">
          <div className="text-2xl font-bold font-mono tabular-nums" style={{ color: catColor }}>{catCount.toLocaleString()}</div>
          <div className="text-[9px] text-gray-600 uppercase tracking-wider mt-0.5">outlets</div>
        </div>
        <div className="rounded-lg bg-white/[0.04] border border-white/[0.05] px-2 py-2.5 text-center">
          <div className="text-2xl font-bold text-white font-mono">#{rank || '—'}</div>
          <div className="text-[9px] text-gray-600 uppercase tracking-wider mt-0.5">of {zonesForCat.length}</div>
        </div>
        <div className="rounded-lg bg-white/[0.04] border border-white/[0.05] px-2 py-2.5 text-center">
          <div className="text-2xl font-bold text-white font-mono">{pct}%</div>
          <div className="text-[9px] text-gray-600 uppercase tracking-wider mt-0.5">of zone</div>
        </div>
      </div>
      ...
      {catBrands.length > 0 && (
        <div className="mb-4">
          <div className="text-[10px] text-gray-600 uppercase tracking-widest mb-2">Top {filter} brands here</div>
      ...
      {compareZones.length > 0 && (
        <div className="mb-4">
          <div className="text-[10px] text-gray-600 uppercase tracking-widest mb-2">Zone comparison</div>
          <div className="space-y-1.5">
            {compareZones.map(z => {
              const isCurrent = z.zone === zone.zone
              return (
                <div key={z.zone} className="flex items-center gap-2">
                  <span className={`text-[10px] w-[90px] truncate flex-shrink-0 ${isCurrent ? 'text-white font-semibold' : 'text-gray-500'}`}>
                  ...
                  <span className={`text-[10px] font-mono w-7 text-right flex-shrink-0 ${isCurrent ? 'text-white' : 'text-gray-600'}`}>{z.count}</span>
                </div>
              )
            })}
          </div>
...
export default function BrandIntelligenceMap() {
  ...
  return (
    <section className="relative w-full bg-[#030712] overflow-hidden">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -top-32 left-1/4 w-[600px] h-[600px] rounded-full bg-[#FF5200]/6 blur-[140px]" />
        <div className="absolute bottom-0 right-1/4 w-[500px] h-[500px] rounded-full bg-[#E4002B]/5 blur-[120px]" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          ...
          {data && (
            <div className="flex gap-3 flex-wrap sm:flex-nowrap sm:justify-end">
              {[
                ...
                <div key={label} className="flex flex-col items-center px-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08]">
                  <span className="text-xl font-bold text-white font-mono tabular-nums"><AnimCount target={n} /></span>
        ...
        <div className="grid lg:grid-cols-[58%_1fr] gap-5 items-start">
          ...
          <div className="relative rounded-2xl overflow-hidden border border-white/[0.07] bg-white/[0.02]">
            ...
            <div className="p-3 sm:p-5">
              {loading ? (
                <div className="flex items-center justify-center h-[460px] text-gray-700 text-sm">
          ...
          <div className="flex flex-col gap-4">
            {activeZone ? (
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="text-[10px] text-gray-600 uppercase tracking-widest mb-1">Selected zone</div>
                    <h3 className="text-xl font-bold text-white">{ZONE_POS[activeZone.zone]?.label ?? activeZone.zone}</h3>
                  </div>
                  {filter ? (
                    <button
                      onClick={() => setFilter(null)}
                    ...
                    <div className="flex flex-col items-end gap-1">
                      ...
                      <div className="text-[10px] text-gray-600 uppercase tracking-wider">outlets</div>
                    </div>
                  )}
                </div>
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-2 mb-4">
                      {[{ v: activeZone.totalBrands, l: 'brands' }, { v: activeZone.categories.length, l: 'categories' }].map(({ v, l }) => (
                        <div key={l} className="rounded-lg bg-white/[0.04] border border-white/[0.05] px-3 py-2">
                          <div className="text-lg font-bold font-mono text-white">{v.toLocaleString()}</div>
                          <div className="text-[10px] text-gray-600 uppercase tracking-wider">{l}</div>
                        </div>
                      ))}
                    </div>
                        ...
                        <div key={c.category} style={{ animationDelay: `${i * 40}ms` }}>
                          <div className="flex justify-between items-center mb-1">
                            <button
                              ...
                              className="text-xs text-gray-400 truncate max-w-[160px] flex items-center gap-1.5 hover:text-white transition-colors"
                            >
                              <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: CAT_COLOR[c.category] ?? '#FF5200' }} />
                              {c.category}
                            </button>
                            <span className="text-xs font-mono text-gray-400 ml-2 flex-shrink-0">{c.count.toLocaleString()}</span>
                          </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {activeZone.topBrands.slice(0, 9).map(b => (
                        <span key={b} className="px-2 py-0.5 rounded-full text-[10px] text-gray-400 bg-white/[0.04] border border-white/[0.06]">{b}</span>
                      ))}
                    </div>
              ...
              <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-5 flex items-center justify-center h-48 text-gray-700 text-sm">
            ...
            <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
              <div className="px-4 py-2.5 border-b border-white/[0.06] flex items-center justify-between">
                <span className="text-[10px] text-gray-600 uppercase tracking-widest">All neighbourhoods</span>
                <span className="text-[10px] text-gray-700">{data?.zones.length ?? 0} zones</span>
              </div>
              <div className="overflow-y-auto max-h-[280px] divide-y divide-white/[0.04]"
                ...
                {(data?.zones ?? []).map(z => {
                  ...
                  return (
                    <button key={z.zone} onClick={() => setSelected(z.zone)}
                      className={`w-full flex items-center gap-3 px-4 py-2 text-left transition-all duration-150 ${isSel ? 'bg-white/[0.06]' : 'hover:bg-white/[0.03]'}`}>
                      ...
                      <span className={`text-xs flex-1 ${isSel ? 'text-white font-medium' : 'text-gray-500'}`}>{z.zone}</span>
        ...
        {/* Bottom teaser */}
        <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-2xl border border-white/[0.06] bg-white/[0.02]">
          <div className="flex items-center gap-3 flex-1 w-full sm:w-auto">
            <div className="flex-1 flex items-center gap-3 px-4 py-2.5 rounded-xl border border-white/[0.08] bg-white/[0.03] cursor-not-allowed">
              <svg className="w-4 h-4 text-gray-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
              <span className="text-sm text-gray-600">Search your brand across Bangalore…</span>
